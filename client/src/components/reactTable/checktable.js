...
const CommonCheckTable = (props) => {
    ...
    const data = useMemo(() => {
        if (AdvanceSearch) {
            return searchDisplay ? searchedDataOut : (tableData || []);
        }
        return displaySearchData ? searchedData : (tableData || []);
    }, [searchDisplay, displaySearchData, AdvanceSearch, searchedDataOut, searchedData, tableData]);
    ...
    const handleAdvanceSearch = (values) => {
        ...
        const searchResult = AdvanceSearch ? dispatch(getSearchData({ values: values, allData: tableData, type: title })) : tableData?.filter(item => {
    ...
    useEffect(() => {
        if (columns?.length > 0) {
            const tempCsvColumns = columns
                .filter((col) => col?.Header !== '#' && col?.Header !== 'Action')
                .map((field) => ({ Header: field?.Header, accessor: field?.accessor }));
                ...
                <Grid templateColumns="repeat(12, 1fr)" gap={2}>
                    <GridItem colSpan={{ base: 12, md: 8 }} display={"flex"} alignItems={"center"}>
                        <Flex alignItems={"center"} flexWrap={"wrap"}>
                            ...
                            {customSearch !== false && <CustomSearchInput setSearchbox={setSearchboxOutside ? setSearchboxOutside : setSearchbox} setDisplaySearchData={setSearchboxOutside ? props.setSearchDisplay : setDisplaySearchData} searchbox={searchboxOutside ? searchboxOutside : searchbox} allData={tableData} dataColumn={columns} onSearch={handleSearch} setGetTagValues={props.setGetTagValuesOutside ? props.setGetTagValuesOutside : setGetTagValues} setGopageValue={setGopageValue} />}
                            {
